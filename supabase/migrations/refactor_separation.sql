...
--   - Table `team_members` exists (spbu_id, ...)
...
-- ═══════════════════════════════════════════════════════════════════════════════
DROP FUNCTION IF EXISTS public.get_user_spbu_id();
...
--    Mengecek status kuota pengisian BBM kendaraan hari ini
-- ═══════════════════════════════════════════════════════════════════════════════
DROP FUNCTION IF EXISTS public.fn_check_plate_status(text, text, uuid);
...
BEGIN
  ...
  -- Hitung total liter dan jumlah transaksi hari ini untuk plat ini
  SELECT 
    COALESCE(SUM(liter), 0),
    COUNT(id)
  INTO v_total_liter, v_count_today
  FROM public.transaksi_pertalite
  WHERE plat_nomor = UPPER(TRIM(p_plat))
    AND spbu_id = v_spbu_id
    AND waktu_pencatatan >= date_trunc('day', NOW())
    AND waktu_pencatatan < date_trunc('day', NOW()) + INTERVAL '1 day';

  -- Tentukan apakah sudah melebihi kuota
  IF v_is_motor THEN
    v_has_refueled := v_total_liter >= 5;
    v_remaining_quota := GREATEST(0, 5 - v_total_liter);
  ELSE
...
GRANT EXECUTE ON FUNCTION public.fn_check_plate_status(text, text, uuid) TO authenticated;


-- ═══════════════════════════════════════════════════════════════════════════════
-- 3. RPC: fn_safe_insert_transaction(p_plat, p_liter, p_jenis, p_shift)
--    Server-side price calculation + atomic quota enforcement
-- ═══════════════════════════════════════════════════════════════════════════════
DROP FUNCTION IF EXISTS public.fn_safe_insert_transaction(text, numeric, text, integer);
DROP FUNCTION IF EXISTS public.fn_safe_insert_transaction(text, numeric, text, integer, numeric);
...
BEGIN
  ...
  -- ── ATOMIC QUOTA CHECK (with row locking to prevent race condition) ──
  ...
  -- Hitung total liter dan jumlah transaksi hari ini
  SELECT 
    COALESCE(SUM(liter), 0),
    COUNT(id)
  INTO v_total_liter_today, v_count_today
  FROM public.transaksi_pertalite
  WHERE plat_nomor = v_plat_clean
    AND spbu_id = v_spbu_id
    AND waktu_pencatatan >= date_trunc('day', NOW())
  ...
  -- Enforce kuota
  IF v_is_motor THEN
    IF (v_total_liter_today + p_liter) > 5 THEN
      RETURN json_build_object(
        'success', false,
        'reason', 'quota_exceeded',
        'message', format('Kuota Motor terlampaui! Sisa kuota: %s Liter. Permintaan: %s Liter.', 
          GREATEST(0, 5 - v_total_liter_today), p_liter)
      );
    END IF;
  ELSE
    IF v_count_today > 0 THEN
      RETURN json_build_object(
        ...
        'message', 'Kendaraan Mobil hanya boleh mengisi BBM 1x per hari!'
      );
    END IF;
  END IF;
...
DROP FUNCTION IF EXISTS public.get_dashboard_summary(text, uuid);
...
BEGIN
  ...
  IF v_feed = '[]'::json AND p_filter = 'today' THEN
    SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO v_feed
    FROM (
      SELECT id, plat_nomor, liter, harga, jenis_kendaraan, waktu_pencatatan, operator_email
      FROM public.transaksi_pertalite
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_summary(text, uuid) TO authenticated;
...
-- ═══════════════════════════════════════════════════════════════════════════════
DROP FUNCTION IF EXISTS public.get_export_transactions(text, text, uuid);
...
GRANT EXECUTE ON FUNCTION public.get_export_transactions(text, text, uuid) TO authenticated;
